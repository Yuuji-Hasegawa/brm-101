...
if (function_exists('wpcf7_add_form_tag')) {
    ...
    function wpcf7_checkbox_form_mytag_handler($tag)
    {
        ...
        if ('mycheckbox' == $tag->basetype) {
            $my_type = 'checkbox';
            $my_class = 'o-bx c-frm-chk u-mr-s';
        } else {
            $my_type = 'radio';
            $my_class = 'o-bx c-frm-rdo u-mr-s';
        }
        $wrap_class = 'c-prg-l u-cur-ptr u-dsp-ifx u-flx-y-ctr';
        ...
        $html = sprintf(
            '<span class="wpcf7-form-control-wrap %1$s"><span class="o-cls" %2$s>%3$s</span>%4$s</span>',
...
<?php
    ...
    function wpcf7_pp_acceptance_form_tag_handler($tag)
    {
        ...
        $wrap_class = 'u-flx-y-ctr u-dsp-flx u-cur-ptr u-mb-l';
        $item_atts = array();
        $item_atts['class'] = 'o-bx c-frm-chk u-mr-s u-flx-shn';
        $item_atts['type'] = 'checkbox';
        $item_atts['id'] = 'pp-acceptance';
        ...
        $item_atts = wpcf7_format_atts($item_atts);
        $content = '<label class="c-prg-l u-w-max" for="pp-acceptance"><a class="c-a c-a--und u-hit-exp" target="_blank" rel="noopener" href="' . home_url('/privacy-policy/') . '">プライバシーポリシー</a>に同意する</label>';
        $html = sprintf('<div class="' . $wrap_class . '"><input %1$s />' . $content . '</div>', $item_atts);
        $atts = wpcf7_format_atts($atts);
        $html = sprintf(
